...
class RouteBuses(APIView):
    ...
    def get(self, request):
        ...
        if data is None:
            try:
                start_stop = Stop.objects.get(name=start)
                stop_stop = Stop.objects.get(name=end)
            except Stop.DoesNotExist:
                return Response(data={"message": "Nothing to show"}, status=404)

            routes_between_stops = Route.route_manager.get_bus_in_routes(
                start_stop_id=start_stop.id, end_stop_id=stop_stop.id
            )

            buses_between_stops, routes_id_list = Bus.read_only_manager.get_busid_list_from_routeid_list(
                routes_between_stops
            )

            route_stop_sequenceind = [
                (Route.route_manager.get_ETA_to_seq(rid, RouteStops.read_manager.get_sequence_id_from_route_stop(
                    rid, stop_stop.id
                )) , Route.route_manager.get_ETA_to_seq(rid, RouteStops.read_manager.get_sequence_id_from_route_stop(
                    rid, start_stop.id
                )))  for rid in routes_id_list
            ]
            buses = Bus.read_only_manager.get_buses_from_flat_list(
                    buses_between_stops)
            ...
            for inst, bus, delta in zip(data, buses, route_stop_sequenceind):
                inst['start_time'] = str(datetime.combine(bus.scheduled_date, bus.start_time) + delta[0])
                inst['end_time'] = str(datetime.combine(bus.scheduled_date, bus.start_time) + delta[1])
